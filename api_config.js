...
// Nova função exclusiva para puxar texto simples / HTML (Usado pela 100hitz)
async function safeFetchText(url, forceProxy = false) {
    const cacheKey = url.replace(/([?&])t=\d+/, '');
...
export async function fetchStationData(station) {
    ...
    try {
        ...
        // Processa endpoint "100hitz" focado em extração de texto (Text/HTML)
        if (station.type === '100hitz') {
            const rawText = await safeFetchText(timestampedUrl, needsProxy);
            // Limpa o texto de quaisquer tags HTML ocultas e espaçamentos vazios
            const cleanText = rawText.replace(/<[^>]*>?/gm, '').trim();
            
            if (cleanText && cleanText.includes(' - ')) {
                const parts = cleanText.split(' - ');
                const extArtist = parts[0].trim();
                const extTitle = parts.slice(1).join(' - ').trim();
                
                songObj = { title: extTitle, artist: extArtist, art: station.defaultArt };
                historyObj = [{
                    song: { title: extTitle, artist: extArtist },
                    timestamp: null // Fallback para cache local de histórico
                }];
            } else if (cleanText && cleanText.length > 0) {
                songObj = { title: cleanText, artist: station.name || "100hitz", art: station.defaultArt };
            }
        } 
        // Processa as demais endpoints focadas em JSON
        else {
        ...
        // Validação e limpeza de histórico Global
