...
export async function fetchStationData(station) {
    ...
    try {
        ...
        // Adiciona 100hitz à lista de necessidade de Proxy (CORS estrito)
        const needsProxy = station.type === 'shoutcast' || station.type === '100hitz' || station.useCorsProxy === true;
        ...
        if (station.type === 'azuracast') {
            ...
            // Algumas proxy scripts retornam HTML/Texto Puro caso a formatação do Metadados ICY venha quebrada.
            // Nossa melhoria no 'safeFetchJson' captura isso na variável '_rawText'.
            let rawTitle = '';
            ...
            // Remove tags HTML soltas que certos players injetam
            rawTitle = rawTitle.replace(/<[^>]*>?/gm, '').trim();

            let extArtist = '';
            let extTitle = rawTitle;

            // Extrai Artista e Título baseado no hífen padrão Shoutcast
            ...
            // Este tipo de endpoint retorna apenas Now Playing (não tem array de histórico). 
            // O histórico local (LocalHistory) preencherá automaticamente as faixas passadas com base no tempo de escuta.
        }
